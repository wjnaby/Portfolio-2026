...
class LanyardErrorBoundary extends React.Component {
  ...
  render() { return this.state.error ? <FallbackLanyard /> : this.props.children; }
}

const BASE = typeof import.meta.env.BASE_URL === 'string' ? import.meta.env.BASE_URL : '/';
const CARD_GLB_URL = `${BASE}card.glb`.replace(/\/+/g, '/');
const LANYARD_PNG_URL = `${BASE}lanyard.png`.replace(/\/+/g, '/');
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }) {
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current) {
      ...
      if (band.current?.geometry) band.current.geometry.setPoints(curve.getPoints(isMobile ? 16 : 32));
